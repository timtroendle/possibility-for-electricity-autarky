...
Finally, we assess the necessary potential to reach electricity autarky. Not only allows this to understand the implications of electricity autarky in terms of the amount of land that needs to be used for electricity generation, but also it removes uncertainty from the assessment. Quantifying the potential for every region in Europe has large uncertainties: the assessment is very sensitive to some of the assumptions which in turn may vary between regions in Europe and which are highly uncertain, in particular the amount of available land that can be used for electricity generation [@Holtinger:2016]. When we assess the necessary potential, we do not need to make assumptions on fraction of the land of a region which can be used for electricity generation.
...
```table
---
caption: 'Fraction of regions with insufficient supply and fraction of people living in regions with insufficient supply when considering electricity autarky and given the technical potential. Dense regions have a population density of 1000 people per km^2^ or more. {#tbl:impacts-overview-technical}'
alignment: LRRRR
...
The result of assessing the necessary potential of electricity autarky and land demand it implies are shown in Table @tbl:necessary-land-100 as fractions of the non-built-up areas in each administrative level which are needed for electricity generation. Land demand is generally very low: on the European and national level it is always smaller than 1% of the non-built-up areas and thus almost negligible. On the subnational and municipal level there are some regions which need all or more of their non-built-up area, but in average the land demand is very low as well. The reason for such limited land needs is the abundant source of electricity from roof-mounted PV which is in many cases able to fulfil the electricity demand on its own.
...
Many electricity scenarios for Europe consider much lower shares of PV and roof-mounted PV for reasons we will discuss later: in [@EuropeanCommission:2016a; @FraunhoferIEE:2017; @Greenpeace:2012] for example, the share of PV is below 40%. When we consider 40% as a hard limit to the generation of electricity from roof-mounted PV in each region, we obtain the results as given in Table @tbl:necessary-land-40. Compared to the unconstrained case land demand is much higher on the subnational and municipal level, where it increases on average by factor 5. For all Europe and single nations, it remains relatively low with only 2% of all European built-up areas needed in the former case, and up to 10% for single nations. Figure @fig:necessary-land-map-40 visualises the land demand of all regions on all four administrative levels. It shows how land demand becomes more concentrated the smaller the autarky level becomes as generation is moving closer to demand centres.
